...
pub struct LatencyMonitor {
    advertised_version_timestamps: Arc<RwLock<BTreeMap<u64, (Instant, u64)>>>, // The timestamps when advertised versions were first seen
...
impl LatencyMonitor {
    ...
    fn update_latency_metrics(&self, highest_synced_version: u64) {
        ...
        for (synced_version, (seen_time, seen_timestamp_usecs)) in
            self.advertised_version_timestamps.read().iter()
        {
    ...
    fn update_advertised_version_timestamps(
        ...
        self.advertised_version_timestamps.write().insert(
            highest_advertised_version,
...
mod tests {
    ...
    fn get_advertised_version_timestamps(
        ...
        let advertised_version_timestamps = latency_monitor.advertised_version_timestamps.read();
        let (time_now_instant, timestamp_now_usecs) = advertised_version_timestamps
    ...
    fn verify_advertised_version_timestamps_length(
        ...
        assert_eq!(
            latency_monitor.advertised_version_timestamps.read().len(),
