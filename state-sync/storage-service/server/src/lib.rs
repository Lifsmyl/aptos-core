...
const CACHED_SUMMARY_UPDATE_CHANNEL_SIZE: usize = 100;
...
impl<T: StorageReaderInterface> StorageServiceServer<T> {
    ...
    async fn spawn_storage_summary_refresher(
        &mut self,
        cached_summary_update_notifier: Sender<CachedSummaryUpdateNotification>,
    ) {
...
fn refresh_cached_storage_summary<T: StorageReaderInterface>(
    ...
    *cached_storage_server_summary.write() = storage_server_summary.clone();
    ...
    if let Err(error) = cached_summary_update_notifier
        .try_send(CachedSummaryUpdateNotification::new(highest_synced_version))
    {
        error!(LogSchema::new(LogEntry::StorageSummaryRefresh)
            .error(&Error::StorageErrorEncountered(error.to_string()))
