...
module coin_listing {
    ...
    /// Purchase out right an item from an auction or a fixed price listing.
    ...
    public entry fun bid<CoinType>(
        ...
        if (auction_listing.auction_end_time < minimum_end_time) {
    ...
    public entry fun complete_auction<CoinType>(
        ...
        let (purchaser, price, royalties, commission)  = if (option::is_some(&current_bid)) {
            let Bid { bidder, coins } = option::destroy_some(current_bid);
            ...
            (bidder, price, royalty_charge, commission_charge)
        } else {
            option::destroy_none(current_bid);
            (seller, 0, 0, 0)
        };

        listing::complete(object, purchaser);
...
module listing_tests {
    ...
    fun test_not_started_fixed_price(
        ...
        let listing = coin_listing::init_fixed_price<AptosCoin>(
            seller,
    ...
    fun test_not_started_auction(
        ...
        let token = mint(seller);
        let fee_schedule = fee_schedule(marketplace);
        let listing = coin_listing::init_auction<AptosCoin>(
            seller,
            object::convert(token),
            fee_schedule,
            timestamp::now_seconds() + 1,
    ...
    inline fun fixed_price_listing_for_tokenv1(
        ...
        let fee_schedule = fee_schedule(marketplace);
        let listing = coin_listing::init_fixed_price_for_tokenv1<AptosCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
    ...
    inline fun auction_listing_for_tokenv1(
        ...
        let (creator_addr, collection_name, token_name, property_version) =
            tokenv1::get_token_id_fields(&token_id);
        let fee_schedule = fee_schedule(marketplace);
        let listing = coin_listing::init_auction_for_tokenv1<AptosCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
