...
module marketplace::listing {
    ...
    public entry fun extract_tokenv1(
        ...
        let object_addr = object::object_address(&object);
        assert!(object::is_owner(object, signer::address_of(owner)), 0);
        let TokenV1Container {
            token,
            delete_ref: _,
            transfer_ref: _,
    ...
    public fun compute_royalty(
        ...
        if (exists<TokenV1Container>(obj_addr)) {
            ...
            let royalty_amount = amount * numerator / denominator;
            ...
            if (option::is_some(&royalty)) {
                ...
                let royalty_amount =
                    amount * royalty::numerator(&royalty) / royalty::denominator(&royalty);
                (payee_address, royalty_amount)
            } else {
                (@0x0, 0)
            }
